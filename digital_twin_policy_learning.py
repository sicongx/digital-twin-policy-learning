...
If transition_fn is None, the action column is updated and any configured cumulative
or time-since-action columns are updated automatically.
...
class RNNModel(nn.Module):
    """LSTM-based sequence model used as the digital twin / microsimulation engine.
...
class GenericTrajectoryEnv:
    ...
    def step(self, action: int) -> Tuple[np.ndarray, State, float, bool]:
        ...
        Default behavior:
        - If transition_fn is None, the environment updates the action column and any
        configured cumulative/time-since-action columns automatically.
...
class TrajectoryDataset:
    ...
    def _default_history_start_idx(actions: np.ndarray) -> int:
        ...
        By default, simulation begins at the second observed action if at least two actions
        are present, otherwise at the first observed action, otherwise at index 0.
        """
    ...
    def from_long_format(
        ...
        time_since_action_col : str, optional
            Covariate column that should track time since the last action and be updated
            during default simulation transitions.
        time_since_action_state_col : str, optional
            RL state column corresponding to time since action.
        time_since_action_state_bins : Sequence[int], optional
            Bin edges used to discretize time since action when refreshing RL state.
